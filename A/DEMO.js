...
~(function (window) {
     ...
     * @constructor
     */
    var DEMO = function(selector, context){
        //如果selector 为方法则为窗口添加页面加载完成事件监听
        if(typeof selector == 'function'){
            DEMO(window).on('load', selector);
        }else {
            //创建DEMO对象
            return new DEMO.fn.init(selector, context);
    ...
    //设置构造函数原型
    DEMO.fn.init.prototype = DEMO.fn;
     ...
     * @type {DEMO.extend}
     */
    DEMO.extend = DEMO.fn.extend = function () {
        var i = 1,
    ...
    DEMO.extend({
        ...
        /**
         * @name 创建一个元素并包装成DEMO的对象
         * @param type  元素类型
         * @param value 元素属性对象
         */
        create : function (type, value) {
            var dom = document.createElement(type);
            return DEMO(dom).attr(value);
        },
        /**
         * @name 格式化模板
         * @param str   模板字符串
